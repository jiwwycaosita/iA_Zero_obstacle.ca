...
## Contenu du dépôt
- `main.py` : serveur FastAPI avec agents orchestrateur, extraction PDF, admissibilité, préremplissage et démos.
...
## Notes
- L'orchestration s'appuie sur l'API Ollama (`/api/generate`). Assurez-vous qu'Ollama est installé et qu'un modèle (par défaut `llama3.1`) est disponible.
...
- La démo `/demo/pdf_extraction` utilise un mini-PDF encodé en base64 embarqué pour faciliter les tests rapides.
